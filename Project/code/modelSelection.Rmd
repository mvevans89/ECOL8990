...
Split into Two Dataframes. Do not include those that didn't lay eggs, as this can be for lots of reasons.

```{r}
aeFec <- dplyr::filter(fecundity, Species == "Aedes" & !is.na(Eggs))
...
Calculate growth rate. Using raw data except for fecundity (since this was only estimated from a subset). Fecundity is estimated using models above.
```{r}
...
GrowthAedes <- emergenceData %>%
  ...
  #drop those when no emerged since we will be summing
  filter(!is.na(Ax) & Ax>0) 
...
GrowthAedes <- GrowthAedes %>%
  ...
  mutate_at(c("Axfwx", "xAxfwx"), sum) %>%
  mutate(D=14) %>%
  ungroup() %>%
  mutate(r=(log((1/N0)*Axfwx))/(D+(xAxfwx/Axfwx))) %>%
  group_by(Replicate, Temp, Ratio, AeDens, StDens, Species) %>%
