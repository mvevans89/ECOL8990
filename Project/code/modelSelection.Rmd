...
ggplot(data = stSurvival, aes(y = (success/(failure+success))))+
  geom_point(aes(x = jitter(AeDens), color = Temp))+
  geom_smooth(method="loess", aes(color = Temp, x = AeDens), se = F)
...
m6 <- glmer(cbind(success, failure) ~ (StDensScale + AeDensScale)*poly(TempScale,2) + (1|Replicate),
                   data = stSurvival,
                   family = binomial(link = "logit"))

m7 <- glmer(cbind(success, failure) ~ poly(TempScale,2) + AeDensScale + (1|Replicate),
                   data = stSurvival,
                   family = binomial(link = "logit"))
```
...
StephSurvivalModel <- m6
```

For stephensi, model m6 is the best fit (survival ~ (AeDens + StDens)*Temp^2)
...
save(AedesSurvivalModel, AedesFecundityModel, AedesGrowthModel, 
     ...
     StephensiGrowthModel2, file = "finalModels.RData")
