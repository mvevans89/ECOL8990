...
Whether or not growth is zero (basically mosquitoes emerge), is dependent on density, not temperature.
...
Calculate overall prediction of growth rate by back transforming results of two models see https://stats.stackexchange.com/questions/234887/predicting-future-values-with-hurdle-poisson-model , but adjust for gamma

```{r}
phi_zero <- predict(StephensiGrowthModelBin, newdata = GrowthStephensi, type = "response")
gammaPreds<- predict(StephensiGrowthModel2, newdata = GrowthStephensi, type = "response")
#get >0 probability of second model
phi_count <- pgamma(q = 0, 
                    shape = 1/sigma(StephensiGrowthModel2)^2,
                    scale = gammaPreds/2,
                    log.p = F,
                    lower.tail = F)
...
predsAll <- phi * gammaPreds #final prediction over all
