...
abline(h=0, col="red")
abline(v=MAPvar$dist.class[which(MAPvar$coef<0.05)[1]], col="red")
...
abline(h=0, col="red")
abline(v=FIREvar$dist.class[which(FIREvar$coef<0.05)[1]], col="red")
```
...
treeThin <- treeData
while(min(distance, na.rm = T) < 1 & nrow(treeThin) > 1){ #loop until each one is gone
  nearNb <- which(distance < 1, arr.ind = T)[,1]
...
This results in a very small data frame of only 9 samples.
...
The thinned model has a significant negative effect of fire, while the model including exponential correlation did not.